...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    true_paragraphs = [paragraph for paragraph in paragraphs if select(paragraph)]
    return true_paragraphs[k] if k < len(true_paragraphs) else ''
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    correct_words = 0
    for index, word in enumerate(typed_words[:len(source_words)]):
        if index < len(source_words) and word == source_words[index]:
            correct_words += 1
    return correct_words / len(typed_words) * 100 if typed_words else 0.0
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        pair = (typed, source)
        if pair in cache:
            cached_value, cached_limit = cache[pair]
            if limit <= cached_limit or cached_value <= cached_limit:
                return cached_value

        diff_value = diff_function(typed, source, limit)
        cache[pair] = (diff_value, limit)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return_word = min(word_list, key=lambda word: diff_function(typed_word, word, limit))
    return return_word if diff_function(typed_word, return_word, limit) <= limit else typed_word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < 0 or abs(len(typed) - len(source)) > limit:  # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return limit + 1
...
def time_per_word(words, times_per_player):
    ...
    for player_time in times_per_player:
        time.append([curr - prev for prev, curr in zip(player_time[:-1], player_time[1:])])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    players_best_words = [[] for _ in player_indices]
    for word_index in word_indices:
        ...
        players_best_words[fastest_player].append(get_word(match, word_index))
    return players_best_words
    # END PROBLEM 10
