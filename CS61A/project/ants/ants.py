...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        for _ in range(self.lower_bound):
            place = place.entrance if place.entrance else place
        n = self.lower_bound
        while not place.is_hive and not place.bees and n <= self.upper_bound:
            place = place.entrance
            n += 1
        return n <= self.upper_bound and not place.is_hive and random_bee(place.bees) or None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        [bee.reduce_health(amount) for bee in copied_bees]
        copied_bees = self.place.bees[:]
        super().reduce_health(amount)
        self.health <= 0 and [bee.reduce_health(self.damage) for bee in copied_bees]
        # END Problem 5
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        self.can_contain(ant) and setattr(self, "ant_contained", ant)
        # END Problem 8a
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        self.ant_contained and self.ant_contained.action(gamestate)
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        copied_bees = self.place.bees[:]
        [bee.reduce_health(self.damage) for bee in copied_bees]
        self.ant_contained and self.ant_contained.action(gamestate)
# END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        not insect.is_waterproof and insect.reduce_health(insect.health)
        # END Problem 10
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place:
            place.ant and place.ant.double()
            getattr(place.ant, "ant_contained", None) and place.ant.ant_contained.double()
            place = place.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            (target.slow_turns <= 0 or gamestate.time % 2 == 0) and Bee.action(target, gamestate)
            target.slow_turns -= 1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif getattr(self, "scared", 0):
            destination = not self.place.entrance.is_hive and self.place.entrance or self.place
            self.scared -= 1
