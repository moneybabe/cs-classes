...
def flip_two(s):
    ...
    #     curr.first, curr.rest.first = curr.rest.first, curr.first
    #     curr = curr.rest.rest
...
def multiply_lnks(lst_of_lnks):
    ...
    for lnk in lst_of_lnks:
        if lnk is Link.empty:
            return lnk
        multiple *= lnk.first
    return Link(multiple, multiply_lnks([lnk.rest for lnk in lst_of_lnks]))
...
def level_mutation_link(t, funcs):
	...
	Tree(2, [Tree(100)])
	"""
	if t is Link.empty or funcs is Link.empty:
		return
